...
class Decoder:
    ...
    def feature_vector(self, history, tag):
        ...
        # Remove any that are not in the set of features
        for feature in [feature for feature in f]:
            if not feature in self.f:
                f.discard(feature)
...
#         for feature in self.f:
        for feature in f:
            ...
            if token[0] == TAG:
                # Handle cases where a ':' appears as part of the word
                r = feature[feature.index(':')+1:feature.rindex(':')]
                u = token[-1]
                if history['i'] <= len(history['x']):
    ...
    def inner_product(self, g):
        ...
        """
        return sum((self.v[key] * value for key, value in g.iteritems()))
